...
static const PsxrecompCodegenHostConfig kCodegenConfig = {
    ...
    .gen_marker_relpath = "generated/SLES_017.22_dispatch.c",
    .build_dir_name = "build-release",
    .cmake_target = "psx-runtime",
    .exe_basename = "BloodyRoar2_Recompiled",
    .prepare_note =
        "Uses your legal disc with the local psxrecomp SDK to generate "
        "BIOS + game C, then cmake --build. The product lives under "
        "build-release/; reopening this setup exe forwards there. The "
        "Europe region builds by default; the USA region is built from "
        "game_us.toml (see README).",
